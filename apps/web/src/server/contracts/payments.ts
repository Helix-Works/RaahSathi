...
export const paymentProviderSignatureHeaderSchema = z.string().regex(/^sha256=[0-9a-f]{64}$/);
...
export const paymentEndpointContracts: readonly EndpointContract[] = [
  ...
  },
  {
    ...
    requestHeaders: [{ name: "x-raahsathi-provider-signature", description: "HMAC-SHA-256 signature over the canonical synthetic event.", required: true, schema: paymentProviderSignatureHeaderSchema }],
